...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances):
        ...
        for i, line in enumerate(instances):
            ...
            label = line['label']
            offset = line['offset']
            qtypes = ['description'] # ['emo'] #['bbox','landmark','attr','headpose','age_gender_race', 'emo']
            for qtype in qtypes:
                # 单独一轮的 conversation
                ...
                conv.append_message(conv.roles[0], FaceTaskDescription["emo_estim"])
                conv.append_message(conv.roles[1], FaceTaskQuestion["emo_estim"])
                conv.append_message(conv.roles[2], label['emo']) # 把checkpoints/ckpts/RAFDB_91.03/eval/result_all.json的pred按照data_list/anno_json/rafdb_emo_au_val_mini_description.json相同的格式修改 emo 做成待测试的 json
    ...
    for result_dicts, batch_prompts, batch_images, batch_has_images in tqdm(data_loader):
        ...
        with torch.inference_mode():
            output_dict = model.generate(
                ...
                do_sample=False,
                max_new_tokens=800, # why setting 13
                output_scores=True,
